...
AdsModer.openDisableOfficeBox = function(ajaxParams, isCompleteCompany, companyName, companyClearFields) {
  ...
  cur.lang.ads_disable_office_confirm_message              = 'Вы уверены что хотите <b>закрыть</b> рекламный кабинет?<br><br>После закрытия кабинета останется доступ к статистике, но любые <b>активные действия будут невозможны</b>.';
  cur.lang.ads_disable_office_company_info_message         = 'К текущему кабинету прикреплена компания.<br>Наименование: <b>{company_name}</b><br><br>Только после закрытия текущего кабинета компания сможет выставлять счета в новом кабинете.';
  cur.lang.ads_disable_office_company_clear_fields_message = 'При закрытии кабинета следующие поля будут <b>обнулены</b>.';
