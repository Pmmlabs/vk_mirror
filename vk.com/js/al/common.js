...
var nav = {
  ...
  go: function(loc, ev, opts) {
    ...
    if (!opts.nocur) {
      ...
      if (zNav(clone(changed), {hist: opts.hist, asBox: opts.asBox, onDone: opts.onDone}, objLoc) === false) {
        nav.setLoc(strLoc);
        return false;
      }
...
function lockLink(el, moreClasses) {
  ...
  moreClasses && each(moreClasses, function (k, v) { addClass(el, v); });
}
...
function unlockLink(el, moreClasses) {
  ...
  moreClasses && each(moreClasses, function (k, v) { removeClass(el, v); });
}
...
function articleNav(strLoc, oldLoc, isBack, postId) {
  ...
  if (match) {
    ...
    stManager.add([jsc('web/article_layer.js'), 'article.css'], function () {
      ...
      cur.articleLayer = new ArticleLayer({ url: url })
...
function articlePrepare(articleUrl) {
  ...
  stManager.add([jsc('web/article_layer.js'), 'article.css'], function () {
    ArticleLayer.prepare({ url: articleUrl })
  })
}
