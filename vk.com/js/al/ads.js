...
Ads.showAnnouncementVideoBox = function(title, player_link, title_link) {
  ...
  showFastBox({
    ...
    hideButtons: true,
    animSpeed: 300,
    onShow: function() { setStyle(window.boxLayerBG, 'opacity', 0.8); },
    onHide: function() { setStyle(window.boxLayerBG, 'opacity', ''); }
  }, content);
}
  ...
  function onRequestComplete(response) {
    ...
    if (needContinue.length && requestsCount < 5000) {
      ...
      if (lastError) {
        ...
        showMessage(lastMessage, true, function() { nav.reload(); }, unionType);
      }
      unlockDeletion();
    }
    return true;
...
Ads.showRetargetingGroupBox = function(act, addParams, boxParams) {
  ...
  var showOptions = {params: {width: 480, dark: true, hideButtons: hideButtons, bodyStyle: 'padding: 0;'}};
  showOptions.onFail = Ads.onBoxFail;
...
Ads.retargetingUpdateWip = function (unionId) {
  ...
  ajax.post('/ads?act=a_retargeting_update_wip', {union_id: cur.options.unionId, hash: cur.options.wip_hash}, {
  ...
  ajax.post("/ads?act=a_retargeting_copy_group_check_status", options, {
    onDone: function(data) {
      if (data[0] == cur.const['ADS_RETARGETING_QUEUE_COPY_STATUS_DONE']) {
        ...
        setStyle(ge('copy_' + cur.options.groupId + '_progress'), {width: '100%'});
        ge('ads_retargeting_groups_table').innerHTML = data[1];
        Ui.tableInitFilters(geByClass1('ui_table'));
        curBox().hide();
        showDoneBox(getLang('ads_retargeting_group_copy_status_done'), {out: 2000});
      }
      ...
      if (data[0] == cur.const['ADS_RETARGETING_QUEUE_COPY_STATUS_IN_PROGRESS']){
        ...
        setStyle(ge('copy_' + cur.options.groupId + '_progress'), {width: percent + '%'});
      }
    },
    onFail: function(msg) {
      clearInterval(cur.options.checkStatusIntervalId)
...
Ads.retargetingShareGroup = function (groupId, hash) {
  ajax.post('/ads?act=a_retargeting_share_group', {
    ...
    onDone: function(title, message) {
      curBox().hide();
      showFastBox({title: title}, message);
      return true;
    },
    onFail: function(err) {
...
Ads.getRetargetingPriceListActionsTable = function(event, curUnionId, priceListId, hash) {
  ...
  ajax.post('/ads?act=a_retargeting_price_list_actions', ajaxParams, {onDone: onComplete, onFail: onComplete});
...
Ads.saveRetargetingPriceList = function(unionId, hash, priceListId) {
  ...
  ajax.post('/ads.php?act=a_retargeting_price_lists_save', ajaxParams, {onDone: onComplete.pbind(true), onFail: onComplete.pbind(false)});
...
Ads.archiveRetargetingPriceList = function(unionId, hash, priceListId, confirm, box) {
  ...
  ajax.post('/ads.php?act=a_retargeting_price_lists_archive', ajaxParams, {onDone: onComplete.pbind(true), onFail: onComplete.pbind(false)});
  ...
  function onComplete(isDone, response) {
    ...
    if (response.retargeting_group_is_used) {
      ...
      newBox.setButtons(
        ...
        Ads.showRetargetingGroupBox.pbind('usage', {union_id: unionId, group_id: response.retargeting_group_id}),
        getLang('box_cancel')
      );
      return;
    }
...
Ads.showRetargetingPriceListProcessInfoBox = function(event, unionId, priceListId, tech) {
  ...
  var params  = {'union_id': unionId, 'price_list_id': priceListId, 'tech': (tech ? 1 : 0)};
...
Ads.startPriceListUpdate = function(event, btn, startUpdate, unionId, priceListId, priceListUrl, priceListHash) {
  ...
  if (unionId && priceListId && priceListUrl) {
    ...
    ajax.post('/ads.php?act=a_retargeting_price_lists_update', ajaxParams, {onDone: onComplete.pbind(true), onFail: onComplete.pbind(false)});
  }
...
Ads.showRetargetingPriceListStatusErrorBox = function(event) {
  ...
  Ads.showRetargetingGroupBox('price_list_error', {hideButtons: true});
}
...
Ads.showRelevanceScoreTooltip = function() {
  Ads.showNewFeatureTooltip('ads_promoted_posts_relevance_score_tooltip', geByClass1("ads_relevance_score_top"), {width: 190, offset: [10, 0], content: getLang('ads_relevance_score_hint_new_feature')});
}

Ads.showPixelErrorsTooltip = function() {
  Ads.showNewFeatureTooltip('adsPixelErrorsTooltip', geByClass1("ads_retargeting_pixel_errors_column"), {width: 235, offset: [-5, 0], content: getLang('ads_retargeting_pixel_errors_new_feature')});
}

Ads.showPriceListErrorsTooltip = function() {
  Ads.showNewFeatureTooltip('adsPriceListErrorsTooltip', geByClass1("ads_retargeting_price_list_item_cell_status_link"), {width: 235, offset: [-15, 0], content: getLang('ads_retargeting_price_list_errors_new_feature')});
}
