  ...
  stManager.add('auto_list.js', function () {
    ...
    function _showLayer(playlist, isDeleted) {
      ...
      if (isDeleted) {
        ...
        (new MessageBox({title: text[0]})).content(text[1]).setButtons(getLang('global_close'), function() { curBox().hide() }).show();

        return
      }

      // Set location
      var newLoc = extend(nav.objLoc, {z: 'audio_playlist' + playlistOwnerId + '_' + playlistId + (accessHash ? ('/' + accessHash) : '')})
      nav.setLoc(newLoc)
