...
AdsEdit.initUploadPhotoBox = function(uploadBox, uploadUrl, uploadVars, uploadOptions) {
  ...
  uploadOptions = extend({}, uploadOptions, {
    ...
    onUploadProgress: function(uploadIndex, bytesLoaded, bytesTotal) { AdsEdit.drawUploadGradientProgress(uploadBox, bytesLoaded, bytesTotal); }
  });
...
AdsEdit.onUploadPhotoComplete = function(uploadBox, uploadIndex, result) {
  ...
  var resultParsed;
  try { resultParsed = parseJSON(result); } catch (e) {}
...
AdsEdit.initCropPhotoBox = function(cropBox, formatPhotoSize, uploadUrl, uploadParams, cropPhotoBoxWidth, resultPhotoWidth, resultPhotoHeight, previewPhotoWidth, previewPhotoHeight, cropOptions, newSizeWidth, newSizeHeight) {
  ...
  var icons = [{width: previewPhotoWidth, height: previewPhotoHeight, box: 'ads_edit_crop_photo_small'}];
...
AdsEdit.initUploadVideoBox = function(uploadVideoBox, uploadUrl, uploadVars, uploadOptions, updateAfterUploadHash) {
  ...
  uploadOptions = extend({}, uploadOptions, {
    ...
    onUploadProgress: function(uploadIndex, bytesLoaded, bytesTotal) { AdsEdit.drawUploadGradientProgress(uploadVideoBox, bytesLoaded, bytesTotal); }
  });
  Upload.init(uploaderElem, uploadUrl, uploadVars, uploadOptions);
...
AdsEdit.showCreatingPostForm = function(buttonElem, creatingPostBox, postOwnerId) {
  ...
  creatingPostBox = showBox('/adsedit?act=creating_post_form', ajaxParams, {onDone: onComplete.pbind(true), onFail: onComplete.pbind(false)});
...
AdsEdit.initCreatingPostForm = function(creatingPostBox, postOwnerId, wallOptions) {
  ...
  // Pre-load clubs list
  ajax.post('/ads_edit.php', {act: 'a_get_promoted_post_stealth_groups'}, {cache: 1});
...
AdsViewEditor.prototype.init = function(options, editor, targetingEditor, params, paramsData, paramsParams) {
  ...
  // ѕараметры сохранени€ рекламного объ€влени€.
  // value каждого параметра передаЄтс€ на сервер при сохранении объ€влени€ и при обновлении данных через get_target_params.
  // ≈сли какое-то значение не нужно дл€ этих операций, а выполн€ет какую-то второстепенную задачу, то нужно его положить в дополнительное поле наиболее подход€щего по смыслу параметра.
  this.params = {
    ad_id:                      {value: 0},
...
AdsViewEditor.prototype.reduceTitle = function() {
  ...
  title = title.replace(/([\s:,]|[\-ЦЧ\|]\s)[^\s:,]+$/, '');
  title = title.replace(/[\s:,\-ЦЧ\|]+$/, '');
...
AdsTargetingEditor.prototype.initUiCriterion = function(criterionName) {
  ...
  // Init UI control
  // Ќовый case есть смысл дописывать к существующим case-ам инициализации, если новый параметр инициализируетс€ таким же образом как и старый, например age_from и school_from.
  // ≈сли же инициализаци€ сильно отличаетс€, то нужно добавл€ть новый самосто€тельный case, например cities и geo_near.
  switch (criterionName) {
    //
    // Dropdowns
    //
...
AdsTargetingEditor.prototype.getUiCriterionSelectedData = function(criterionName) {
  switch (criterionName) {
    case 'geo_near':
      return ''; // »нициализируетс€ в initGeoEditor
    case 'country':
